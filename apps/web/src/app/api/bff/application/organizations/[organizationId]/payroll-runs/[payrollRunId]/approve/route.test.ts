...
const postPayrollRunApproval = vi.hoisted(() => vi.fn());
vi.mock('../../../../../../../../../lib/auth/bff', () => ({
  postPayrollRunApproval,
}));
vi.mock('../../../../../../../../../lib/auth/server', () => ({ getAuth }));
import { POST } from './route';
describe('approve payroll BFF route', () => {
  it('delegates POST', async () => {
    ...
    postPayrollRunApproval.mockResolvedValue(response);
    expect(
      await POST(request, {
    ...
    expect(postPayrollRunApproval).toHaveBeenCalledWith(
      { marker: 'auth' },
      request,
      'org_1',
