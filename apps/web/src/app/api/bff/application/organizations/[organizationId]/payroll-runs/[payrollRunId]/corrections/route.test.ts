...
const postPayrollRunCorrection = vi.hoisted(() => vi.fn());
vi.mock('../../../../../../../../../lib/auth/bff', () => ({
  postPayrollRunCorrection,
}));
vi.mock('../../../../../../../../../lib/auth/server', () => ({ getAuth }));
import { POST } from './route';
describe('corrections payroll BFF route', () => {
  it('delegates POST', async () => {
    ...
    postPayrollRunCorrection.mockResolvedValue(response);
    expect(
      await POST(request, {
    ...
    expect(postPayrollRunCorrection).toHaveBeenCalledWith(
      { marker: 'auth' },
      request,
      'org_1',
