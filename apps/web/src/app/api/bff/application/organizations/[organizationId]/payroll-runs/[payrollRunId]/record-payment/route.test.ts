...
const postPayrollRunPayment = vi.hoisted(() => vi.fn());
vi.mock('../../../../../../../../../lib/auth/bff', () => ({
  postPayrollRunPayment,
}));
vi.mock('../../../../../../../../../lib/auth/server', () => ({ getAuth }));
import { POST } from './route';
describe('payment payroll BFF route', () => {
  it('delegates POST', async () => {
    ...
    postPayrollRunPayment.mockResolvedValue(response);
    expect(
      await POST(request, {
    ...
    expect(postPayrollRunPayment).toHaveBeenCalledWith(
      { marker: 'auth' },
      request,
      'org_1',
