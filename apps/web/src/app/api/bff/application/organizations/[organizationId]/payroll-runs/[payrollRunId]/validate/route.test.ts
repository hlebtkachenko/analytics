...
const postPayrollRunValidation = vi.hoisted(() => vi.fn());
vi.mock('../../../../../../../../../lib/auth/bff', () => ({
  postPayrollRunValidation,
}));
vi.mock('../../../../../../../../../lib/auth/server', () => ({ getAuth }));
import { POST } from './route';
describe('validate payroll BFF route', () => {
  it('delegates POST', async () => {
    ...
    postPayrollRunValidation.mockResolvedValue(response);
    expect(
      await POST(request, {
    ...
    expect(postPayrollRunValidation).toHaveBeenCalledWith(
      { marker: 'auth' },
      request,
      'org_1',
