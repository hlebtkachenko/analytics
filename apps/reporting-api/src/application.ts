...
import type { ReportingLogger } from './logger.js';
...
export function configureApplication(
  ...
  const adapter = application.getHttpAdapter().getInstance() as {
  ...
  const openApi = new DocumentBuilder()
    .setTitle('BAP Reporting API')
    .setDescription('Private reporting service access contract')
    .setVersion('1.0.0')
    .addBearerAuth(
