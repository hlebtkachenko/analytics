...
CSV/XLSX export, chat, and chart rendering, and the documents analytics page,
whose revenue and expense, VAT balance, and partner charts render Carbon Charts
beside their table views. Other domain-specific analytics, dashboards,
analytical metrics, and business workflows remain deferred.
...
Application icons come from the exact curated 51-export
...
renders those 51 glyphs and all 1,575 pictograms; the generated catalog alone
...
header branded "Afframe Analytics" with single-purpose panels for search, help,
account, and workspace switching, over a pinned-persistable left icon rail for
Workspaces, Datasets, Documents, and Account, and a workspace section, rendered
from the shell's `railDestinations` array. Identity and invitation routes remain
outside that shell. The layout owns the single `main-content` landmark and
