...
if [[ ! -f "$ai_provider_path" ]]; then
  printf '%s\n' '{"provider":"anthropic","apiKey":"local-development-placeholder","models":{"chat":"claude-sonnet-5","embedding":"text-embedding-3-small","summary":"claude-sonnet-5"}}' > "$ai_provider_path"
fi
