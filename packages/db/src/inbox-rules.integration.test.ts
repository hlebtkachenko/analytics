...
afterAll(async () => {
  await endPools(apiPool, migratorPool, reportingPool, rootPool);
...
describe('inbox rules', () => {
  it('applies the inbox rules migration and records the compatible version', async () => {
    ...
    expect(result.currentVersion).toBe('20260923.0001');
    expect(DATABASE_MIGRATION_COMPATIBILITY).toBe('20260923.0001');
    expect(compatibility).toEqual({
      compatible: true,
      expectedVersion: '20260923.0001',
      version: '20260923.0001',
    });
