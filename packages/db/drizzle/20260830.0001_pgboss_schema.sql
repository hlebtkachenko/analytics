-- pg-boss 12.28.1 schema, version 38, generated from getConstructionPlans and pinned here.
...
CREATE INDEX IF NOT EXISTS job_dep_parent_idx ON pgboss.job_dependency (parent_name, parent_id);
  ...
  ;
    ...
    LANGUAGE plpgsql;
  ;
INSERT INTO pgboss.version(version) VALUES ('38');

-- pgboss.job has no row level security, so payloads must carry identifiers only.
...
GRANT USAGE ON SCHEMA pgboss TO bap_api, bap_backup;

-- The worker and the application API share the bap_api role, which cannot create objects.
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA pgboss TO bap_api;
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA pgboss TO bap_api;
ALTER DEFAULT PRIVILEGES IN SCHEMA pgboss GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO bap_api;
ALTER DEFAULT PRIVILEGES IN SCHEMA pgboss GRANT EXECUTE ON FUNCTIONS TO bap_api;

-- Without these the whole-database dump in the backup drill fails.
GRANT SELECT ON ALL TABLES IN SCHEMA pgboss TO bap_backup;
GRANT SELECT ON ALL SEQUENCES IN SCHEMA pgboss TO bap_backup;
ALTER DEFAULT PRIVILEGES IN SCHEMA pgboss GRANT SELECT ON TABLES TO bap_backup;
ALTER DEFAULT PRIVILEGES IN SCHEMA pgboss GRANT SELECT ON SEQUENCES TO bap_backup;
