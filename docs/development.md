...
`(product)/account` and the `[orgSlug]` workspace routes are a deliberate legacy
exception: they use plain semantic HTML and are exempt from the `PageContainer`
rule. New pages, including the Carbon `/organizations` list and create pages,
follow the rule above, not that exception.
