...
| `GET /api/bff/application/organizations/:organizationId/entity-scopes`                                           | `GET http://api:3001/v1/organizations/:organizationId/entity-scopes`                                                 | 3 seconds; bounded JSON                                                                                                         | Requires `manageEntityAccess`; one entry per stored scope row, so the members page reads every scope in one call instead of one per member                                                                                           |
...
explicit set of legal entities. An owner is never restricted. A restricted admin
or member also loses `createEntities`, because it could not see the entity it
had just created.
